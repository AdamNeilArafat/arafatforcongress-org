...
RAW = Path(__file__).resolve().parents[0].parent / "data" / "raw"
RAW.mkdir(parents=True, exist_ok=True)

# Determine the latest even-numbered election cycle
...
files = {
    "cn": f"cn{str(cycle)[-2:]}.zip",   # e.g. cn26.zip
    "cm": f"cm{str(cycle)[-2:]}.zip",   # e.g. cm26.zip
}
