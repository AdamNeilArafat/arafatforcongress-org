...
// Optional offline fallback from config:
// {
//   "candidate_committees": { "H0...": { "2026": ["C00..."] } }
// }
async function getCandidateCommittees(candidateId, cycle) {
  if (!SKIP) return getCandidateCommitteesOnline(candidateId, cycle);
...
function idxByCommitteeCycle(arr) {
  const m = new Map();
  for (const r of arr) m.set(`${r.committee_id}_${r.cycle}`, r);
  return m;
}
...
for (const m of metrics) {
  siteCommittees[m.committee_id] = {
    ...
    latest_coverage_end_date: coverageEnd(m) || null,
    totals_ytd: {
...
for (const cycle of cycles) {
  for (const cid of (cfg.candidate_ids || [])) {
    try {
      ...
      const rows = cmtes.map(k => byKey.get(`${k}_${cycle}`)).filter(Boolean);
      ...
      const merged = {
        ...
        latest_coverage_end_date: (rows.map(coverageEnd).sort().pop()) || null,
        totals_ytd: {
