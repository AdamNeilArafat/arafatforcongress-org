...
import { normalizeRecord, normalizeHeader, fromVoterData, mergeRecord } from './field-ops-v3-modules/data.js';
import { enrichWithGemini, audit } from './field-ops-v3-modules/enrichment.js';
...
const settings=Object.assign({gemini_model:'gemini-2.0-flash-lite',election_date:'2026-11-03',phase:'Primary Push',admin_pin:localStorage.getItem('admin_pin')||'1234'},read(KEYS.settings,{}));
...
async function ingestInitial(){
  ...
  if(!state.records.length){state.records=[normalizeRecord({first_name:'Demo',last_name:'Voter',address:'123 Main',city:'Tacoma',zip:'98402',status:'not_contacted',district:'WA-10'})];}
  persist();
}
...
function renderVoters(){const q=$('#vSearch').value;state.filters={district:$('#fDistrict').value,status:$('#fStatus').value,priority:$('#fPriority').value,assigned:$('#fVolunteer').value};let rows=filterRecords(state.records,q,state.filters);rows=sortRecords(rows,state.sort.key,state.sort.dir);const tb=$('#voterRows');tb.innerHTML=rows.map(r=>`<tr><td><input type='checkbox' data-id='${r.id}' ${state.selected.has(r.id)?'checked':''}></td><td>${r.full_name}</td><td>${r.zip||''}</td><td>${r.phone||''}</td><td>${r.email||''}</td><td>${(r.updated_at||'').slice(0,10)}</td><td><span class='status-pill status-${r.status}'>${r.status}</span></td><td><button class='btn' data-enrich='${r.id}'>Enrich</button><button class='btn' data-status='${r.id}|contacted'>Contacted</button></td></tr>`).join('');$('#filteredCount').textContent=`${rows.length} shown`;}

async function enrichOne(id){const rec=state.records.find(r=>r.id===id);if(!rec) return;try{const patch=await enrichWithGemini(rec,settings);Object.assign(rec,mergeRecord(rec,patch));audit({id,ok:true});persist();renderAll();}catch(e){audit({id,ok:false,error:e.message});alert(e.message==='missing_api_key'?'Add Gemini API key in Settings.':`Enrichment failed: ${e.message}`);} }
...
function importCsv(file){Papa.parse(file,{header:true,skipEmptyLines:true,complete:(res)=>{const mapped=res.data.map(row=>{const n={};Object.entries(row).forEach(([k,v])=>n[normalizeHeader(k)]=v);return normalizeRecord(n);});state.records=[...state.records,...mapped];persist();renderAll();}})}
