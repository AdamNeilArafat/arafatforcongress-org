// /assets/js/accountability.js
// Page bootstrap for Financial Alignments — uses shared modules.

import { getData, getVisibleIdsFromTable } from "./data-loader.js";
import { headshot } from "./images.js";
import { pct, pctStr, money } from "./formatters.js";
...
function computeChips(dRec, aRec, vRec) {
  ...
  const inds = dRec?.industries || {};
  const total = Object.values(inds).reduce((s,v)=>s+(v||0),0);
  if (total) {
    const [topKey, topVal] = Object.entries(inds).sort((a,b)=>b[1]-a[1])[0];
    if ((topVal/total) >= THRESH.INDUSTRY_DOMINATE) chips.push(["chip-danger","fa-industry",`Dominant industry: ${topKey}`]);
  }
...
function rowData(mid, members, donors, awards, votes) {
  const m = members[mid] || {};
  const d = donors[mid] || {};
  const a = awards[mid] || null;
  const v = votes[mid] || null;
  ...
  // top industry
  const inds = Object.entries(d.industries || {});
  inds.sort((x,y)=>y[1]-x[1]);
  const tiKey = inds[0]?.[0] || "—";
  const tiShare = (inds[0]?.[1] || 0) / (inds.reduce((s, [,v])=>s+(v||0), 0) || 1);
  ...
  return {
    ...
    img,
    pacP, smlP, in$, out$,
    topIndustryKey: tiKey,
    topIndustryShare: tiShare,
...
function hookupFilters(donors) {
  ...
  function passesIndustryFilter(mid, wanted){
    ...
    const inds = (donors[mid]?.industries) || {};
  ...
  function applyFilters(pushState = true) {
    ...
    const ls = $("#load-status"); if (ls) ls.textContent = `Showing ${visible} row(s).`;
    ...
    // Recompute headline aggregates after each filter change
    recomputeHeadlines();
  }
  ...
  resetBtn?.addEventListener("click", ()=>{
    ...
    if (chamberSel) chamberSel.value = "";
...
function recomputeHeadlines() {
  const tbody = $("#fa-table");
  const visibleIds = getVisibleIdsFromTable(tbody);
  ...
  const agg = computeAllAggregates(visibleIds, donors, votes);
...
function hookupModal(members, donors, awards, votes) {
  window.openModal = function(mid){
    ...
    // Share buttons (X, Facebook, Reddit, plus native)
    const urls = shareUrls({ url: location.href, title: `${name} — ${seat}`, text: `${name} • PAC ${pct(d.pac_pct||0)}% • Top: ${tiKey==='—'?'—':tiKey}` });
    const $shareX = $("#shareX");
    if ($shareX) $shareX.href = urls.x;

    // Optional: create more buttons if you add them in the modal footer
    // e.g., #shareFacebook, #shareReddit, #shareNative
...
async function boot() {
  const status = $("#load-status"); if (status) status.textContent = "Loading data…";
  const { members, donors, awards, votes, meta } = await getData();
  if (status) status.textContent = "Data loaded.";

  window.__DATA__ = { members, donors, awards, votes, meta }; // for debugging

  // Guards in dev (non-fatal)
  try { assertSchemas({ members, donors, votes, awards }); } catch (err) { console.warn(err); }

  // Build table
  const mids = Object.keys(donors || {});
  const tbody = $("#fa-table");
  if (!mids.length) {
    tbody.innerHTML = `<tr><td colspan="8" class="text-muted">No finance data found.</td></tr>`;
  } else {
  ...
  // Prime headline aggregates once
  applyFilters(false);
}
