...
(function (A) {
  ...
  function enrich(r){
    ...
    (r.donations||[]).forEach(d=>{
      const k=d.industry||'Unknown';
      byInd[k]=(byInd[k]||0)+(d.amount||0);
    });
  ...
  function topIndustry(rec){
    ...
    const [name,pct]=ent[0];
    return pct>=1 ? {name,pct} : null; // show only if >=1%
  }
  ...
  // ---- CARD (keeps your original classes) ----
  ...
  // ---- render & modal (Yes/No mapping for votes) ----
  ...
  A.openModal = function(r){
    ...
    const votes = (r.votes||[]).slice(0,12).map(v=>
      `<li><a href="${v.source_url}" target="_blank" rel="noopener">${v.date}: ${v.title} — <b>${mapYN(v.position)}</b></a></li>`
    ).join('');

    const adminVotes = (r.adminVotes||[]).map(v=>
      `<li>${v.bill_id} — <b>${mapYN(v.vote)}</b></li>`
    ).join('') || '<li>None</li>';

    const donations = (r.donations||[]).slice(0,12).map(d=>
      `<li>$${(d.amount||0).toLocaleString()} · ${d.through||d.donor} (${d.industry||'Unknown'})</li>`
    ).join('');
    ...
    document.getElementById('modal-body').innerHTML = `
      ...
      <h3>Money</h3>
      <ul>${donations || '<li>No donations listed.</li>'}</ul>
      <h3>Vote history</h3>
      <ul>${votes || '<li>No votes on record.</li>'}</ul>
      <h3>Admin-applied votes</h3>
      <ul>${adminVotes}</ul>
    `;
...
/* === Add/replace these helpers + renderers in assets/js/accountability.js === */

// Pick top industry (>=1%) by share
function topIndustry(rec){
  const entries = Object.entries(rec.shares || {}).sort((a,b)=>b[1]-a[1]);
  if (!entries.length) return null;
  const [name, pct] = entries[0];
  return pct >= 1 ? { name, pct } : null;
}

// Color tone for badge chips
function toneForBadge(label){
  const s = (label||'').toLowerCase();
  if (s.includes('oil') || s.includes('fossil') || s.includes('war') || s.includes('aipac')) return 'red';
  if (s.includes('grass') || s.includes('small')) return 'green';
...
// Build one card using your original classes (.card-person, .avatar, .body, .name, .meta, .badge-rail, .pill)
Accountability.cardHTML = function(rec){
  ...
  const seat = rec.district ? `${rec.state}-${rec.district}` : rec.state;
  const ti = topIndustry(rec);
  const tiHtml = ti ? ` • Top: <b>${ti.name}</b> (${ti.pct.toFixed(0)}%)` : '';
  ...
  const pills = badges.map(b=>`<span class="pill ${toneForBadge(b)}">${b}</span>`).join('');

  return `
  <div class="card-person" data-id="${rec.person_id}">
    <img class="avatar" src="${rec.headshot||'/images/default-headshot.jpg'}" alt="${rec.name}" loading="lazy">
    <div class="body">
      <div class="name">${rec.name}</div>
      <div class="meta">${rec.party} • ${rec.chamber} • ${seat}</div>
      <div class="statline">
        <b>$${total.toLocaleString()}</b> total
        • PAC <b>${pacPct}%</b>
        • Small <b>${smallPct}%</b>${tiHtml}
      </div>
      <div class="badge-rail">${pills}</div>
    </div>
  </div>`;
...
// Render list into any container (keeps your original grid/layout intact)
Accountability.renderWall = function(container){
  const rows = Accountability.filteredRows ? Accountability.filteredRows() : (Accountability._data||[]);
  container.innerHTML = rows.map(Accountability.cardHTML).join('');
  container.querySelectorAll('.card-person').forEach(el=>{
    el.addEventListener('click', ()=>{
      const id = el.getAttribute('data-id');
      const rec = (Accountability._index||{})[id] || (Accountability._data||[]).find(r=>r.person_id===id);
      Accountability.openModal(rec);
    });
  });
};

// Modal with Yes/No mapping (Yea→Yes, Nay→No). Keeps your modal shell.
Accountability.openModal = function(rec){
  const mapYN = (p)=> p==='Yea' ? 'Yes' : p==='Nay' ? 'No' : (p||'—');

  const votes = (rec.votes||[]).slice(0,12).map(v=>(
    `<li><a href="${v.source_url}" target="_blank" rel="noopener">${v.date}: ${v.title} — <b>${mapYN(v.position)}</b></a></li>`
  )).join('');

  const adminVotes = (rec.adminVotes||[]).map(v=>(
    `<li>${v.bill_id} — <b>${mapYN(v.vote)}</b></li>`
  )).join('') || '<li>None</li>';

  const donations = (rec.donations||[]).slice(0,12).map(d=>(
    `<li>$${(d.amount||0).toLocaleString()} · ${d.through||d.donor} (${d.industry||'Unknown'})</li>`
  )).join('');
  ...
  document.getElementById('modal-body').innerHTML = `
    <h2>${rec.name}</h2>
    <h3>Overview</h3>
    <p>Party: ${rec.party} • ${rec.chamber} • ${rec.state}${rec.district?'-'+rec.district:''} • Cycle ${rec.cycle||''}</p>
    <h3>Money</h3>
    <ul>${donations || '<li>No donations listed.</li>'}</ul>
    <h3>Vote history</h3>
    <ul>${votes || '<li>No votes on record.</li>'}</ul>
...
// (Optional) if you don’t already expose these:
Accountability.getById = (id)=> (Accountability._index||{})[id];
